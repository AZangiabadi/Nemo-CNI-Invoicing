...
- Writes an "Invoice" sheet in an XLSX with:
    * Summary totals (cost per lab + internal facility fee + total)
...
# Subsidy applied to CDG usage items (10% subsidy => charge 90% of listed usage cost).
# Industry and External Academia have no subsidy.
CDG_USAGE_SUBSIDY_RATE = 0.10
...
def _requests_required() -> None:
    if requests is None:
        raise RuntimeError("The 'requests' library is required for API access. Install it or run without an API token.")
...
def internal_facility_fee_for_group(df_group: pd.DataFrame) -> float:
    ...
    """
    app_ids = set(df_group.get("Application identifier", pd.Series(dtype=str)).dropna().astype(str))
    if not app_ids:
        return 0.0
    return float(max(INTERNAL_FACILITY_FEE_BY_APPLICATION.get(app_id, 0.0) for app_id in app_ids))
...
def write_table(ws, start_row: int, start_col: int, df_table: pd.DataFrame, currency_cols: Iterable[str] = ()) -> int:
...
def create_invoice_workbook(df_group: pd.DataFrame, pi_display_name: str, period_ym: str) -> Workbook:
    ...
    # Add internal facility fee as a separate line item (once per invoice)
    ws.cell(r, 1, value="Internal facility fee").border = _BORDER_THIN
    ...
    for lab in DESIRED_LAB_ORDER:
        ...
        df_lab = df_lab.sort_values(["Project", "Start_dt", "User", "Item_norm"])

        out = df_lab[["Start_dt", "User", "Item_norm", "Type", "Project", "Quantity", "Rate", "Subsidy", "Cost"]].copy()
        out = out.rename(columns={"Start_dt": "Date", "Item_norm": "Description"})

        current_row = write_table(ws, current_row, 1, out, currency_cols=["Subsidy", "Cost"])
    ...
    # Project fees summary (total charges per project) - once per invoice
    if not df_group.empty:
        ...
        proj["Project Total"] = proj[DESIRED_LAB_ORDER].sum(axis=1)

        proj_cols = ["Project", "Application identifier"] + list(DESIRED_LAB_ORDER) + ["Project Total"]
        proj = proj[proj_cols].sort_values(["Project Total", "Project"], ascending=[False, True])
        ...
        current_row = write_table(
            ...
            1,
            proj,
            currency_cols=list(DESIRED_LAB_ORDER) + ["Project Total"],
        )
        ...
        ws.cell(current_row, 1, value="Internal facility fee").border = _BORDER_THIN
        ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, end_column=last_col - 1)
...
def create_invoice_pdf(
    ...
    if not _pdf_available():
        raise RuntimeError("PDF generation requested, but 'reportlab' is not installed.")
    ...
    if logo is None:
        ...
        logo_cell: object = P("Columbia University", ParagraphStyle("logotxt", parent=styleN, fontSize=14, leading=16))
    else:
        logo.hAlign = "RIGHT"
        logo_cell = logo
    ...
    # styles
    styleTitleCenter = ParagraphStyle("TitleCenter", parent=styleTitle, alignment=TA_CENTER)
    ...
    if show_payment_notice:
        ...
        payment_italic = Paragraph(
            ...
            "Due upon 30 days of<br/>"
            "receipt",
            styleRightItalic,
        )
        logo_cell = [logo_cell, Spacer(1, 8), payment_bold, Spacer(1, 8), payment_italic]

    header_tbl = Table(
        [
            [Spacer(1, 1), P("CNI Facility Usage Invoice", styleTitleCenter), logo_cell],
    ...
    summary_data.append(["Internal facility fee", _fmt_money(internal_fee)])
    ...
    # Details by lab
    detail_col_names = ["Date", "User", "Description", "Type", "Project", "Qty", "Rate", "Subsidy", "Cost"]
    ...
    proj_header = [Paragraph("Project", styleSmallBold), Paragraph("Project Type", styleSmallBold)]
    for lab in DESIRED_LAB_ORDER:
    ...
    # Columns: Project, Account Type, Cleanroom, SMCL, Electron Microscopy Lab, Consumable, Project Total
    proj_fracs = [0.44, 0.09, 0.09, 0.08, 0.12, 0.09, 0.09]
    ...
    totals_tbl = Table(
        [
            ...
            ["Internal facility fee", _fmt_money(internal_fee)],
...
def load_and_prepare(csv_path: str) -> pd.DataFrame:
    ...
    required = {"Type", "User", "Item", "Project", "Application identifier", "Start time", "Rate", "Cost", "Quantity"}
    ...
    df["Lab"] = df["Item_norm"].map(TOOL_TO_LAB).fillna("Unmapped")
    ...
    if cdg_mask.any():
        df.loc[cdg_mask, "Subsidy"] = df.loc[cdg_mask, "Cost"] * CDG_USAGE_SUBSIDY_RATE
        df.loc[cdg_mask, "Cost"] = df.loc[cdg_mask, "Cost"] - df.loc[cdg_mask, "Subsidy"]
...
def generate_invoices(
    ...
    if use_api:
        ...
        pi_infos = df["Project"].apply(lambda p: resolve_pi_for_project(str(p), project_map))
    ...
    for (pi_key, period), grp in df.groupby(["PI_key", "Period"]):
        ...
        if generate_pdf:
            if not _pdf_available():
                print("WARNING: reportlab is not installed; skipping PDF generation.", file=sys.stderr)
            else:
                ...
                try:
                    ...
                    print(f"WARNING: Failed to create PDF for {pi_name} {period_label}: {e}", file=sys.stderr)

    return xlsx_created, pdf_created, df


def create_monthly_invoice_zips(outdir: str, df: pd.DataFrame, remove_members: bool = True) -> List[str]:
    """
    Create one ZIP per billing month label containing all invoice files (XLSX/PDF)
    in the output directory for that month.
    ZIP naming: CNI-Nemo-Invoices-{MONTH}.zip (e.g., CNI-Nemo-Invoices-MAR2026.zip)
    Returns absolute paths of created ZIPs.
    If remove_members is True, source invoice files are deleted after successful ZIP creation.
    """
    ...
    for ml in month_labels:
        ...
        zip_path = os.path.abspath(os.path.join(outdir, f"CNI-Nemo-Invoices-{ml}.zip"))
...
def _prompt_token_console() -> Optional[str]:
    ...
    if not sys.stdin or not sys.stdin.isatty():
        return None
    try:
        token = getpass.getpass("Enter your NEMO API token (leave blank to skip): ").strip()
    except Exception:
...
def launch_gui_app() -> None:
    ...
    def run_job():
        ...
        outdir = outdir_var.get().strip() or os.path.join(os.path.dirname(csv_path), "invoices")
    ...
    tk.Label(root, text="NEMO CSV", **label_opts).grid(row=0, column=0, sticky="w", padx=pad_x, pady=pad_y)
    csv_entry = tk.Entry(root, textvariable=csv_var, **entry_opts)
    csv_entry.grid(row=0, column=1, sticky="ew", padx=pad_x, pady=pad_y)
    tk.Button(root, text="Browse...", command=pick_csv).grid(row=0, column=2, padx=pad_x, pady=pad_y)

    tk.Label(root, text="Output Folder", **label_opts).grid(row=1, column=0, sticky="w", padx=pad_x, pady=pad_y)
    ...
    tk.Button(root, text="Browse...", command=pick_outdir).grid(row=1, column=2, padx=pad_x, pady=pad_y)

    tk.Label(root, text="NEMO Base URL", **label_opts).grid(row=2, column=0, sticky="w", padx=pad_x, pady=pad_y)
    base_entry = tk.Entry(root, textvariable=base_var, **entry_opts)
    base_entry.grid(row=2, column=1, sticky="ew", padx=pad_x, pady=pad_y)

    tk.Label(root, text="API Token", **label_opts).grid(row=3, column=0, sticky="w", padx=pad_x, pady=pad_y)
    ...
    tk.Label(root, text="Logo (optional)", **label_opts).grid(row=4, column=0, sticky="w", padx=pad_x, pady=pad_y)
    ...
    tk.Label(root, textvariable=status_var, anchor="w", **label_opts).grid(row=7, column=1, sticky="w", padx=pad_x, pady=14)
...
def main() -> None:
    ...
    ap.add_argument("--csv", required=False, default=None, help="Path to NEMO usage export CSV")
    ap.add_argument(
        "--outdir",
        required=False,
        default=None,
    ...
    ap.add_argument(
        "--no-zip",
        action="store_true",
        help='Disable ZIP generation ("CNI-Nemo-Invoices-{month}.zip").',
    )
    ap.add_argument(
        "--keep-invoice-files",
        action="store_true",
        help="Keep generated .xlsx/.pdf files after ZIP creation (default removes them).",
    )
    ap.add_argument(
        "--no-gui",
        action="store_true",
    ...
    zips_created = [] if args.no_zip else create_monthly_invoice_zips(
        outdir,
        df,
        remove_members=(not args.keep_invoice_files),
    )
